...
export function useUpdateHyundaiInspection() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || "https://back-prueba-qibm.onrender.com";

  const updateInspection = async (id: string, payload: UpdateHyundaiInspectionPayload): Promise<Inspection | null> => {
    setLoading(true);
    setError(null);

    try {
      const response = await fetch(`${BACKEND_URL}/inspections/hyundai/${id}`, {
        method: "PUT",
        headers: {
