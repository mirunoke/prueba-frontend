...
export function useUpdateMazdaInspection() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || "https://back-prueba-qibm.onrender.com";

  const updateInspection = async (id: string, payload: UpdateMazdaInspectionPayload): Promise<Inspection | null> => {
    setLoading(true);
    setError(null);

    try {
      const response = await fetch(`${BACKEND_URL}/inspections/mazda/${id}`, {
        method: "PUT",
        headers: {
