...
interface Step3Props {
  ...
  setBatteryImage: (f: File | null) => void;
  onNext: () => void;
  onBack: () => void;
}
...
const Step3BatteryCheck: React.FC<Step3Props> = ({
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-semibold text-gray-800 dark:text-white mb-6">Inspección de batería</h2>
      <div className="rounded-lg border p-6 space-y-8">
        <div className="text-center">
          <div className="flex items-center justify-center mb-2">{getBatteryIcon()}<span className="ml-2 text-xl font-semibold">{batteryCharge}%</span></div>
          <div className={`inline-block px-4 py-2 rounded-full ${getBatteryLevelClass()} text-sm font-medium`}>{getBatteryStatus()}</div>
        </div>
        <div className="mt-8">
          <label className="block text-sm font-medium mb-2">Nivel de carga de la batería</label>
          <Slider
            size="lg"
            step={1}
            minValue={0}
            ...
            classNames={{base:"max-w-md mx-auto",track:"h-3",filler:"h-3"}}
          />
        </div>
        <div className="mt-6 bg-primary-50 dark:bg-primary-400 p-4 rounded-lg border border-primary-100">
          <h3 className="text-md font-semibold mb-2 text-primary dark:text-white">Instrucciones</h3>
          <p className="text-primary text-sm dark:text-white">Mueve el slider para indicar el nivel de carga de la batería del vehículo Mazda.</p>
        </div>
      </div>
      <ImageDropzone
        files={batteryImage ? [batteryImage] : []}
        setFiles={(files)=>setBatteryImage(files[0]||null)}
        label="Fotografía de la batería (opcional)"
      />
